...
**Status: verified against current hostap mainline as of 2026-08-25** (task 399, step 1). See `MAINLINE_TRIAGE_TASK399.md` in this directory for the per-item still-present/already-fixed/changed breakdown — 96 of 109 items are still present verbatim, 8 are already fixed upstream (do not include those in any disclosure), 3 have relocated/partially-refactored code with the underlying defect intact, 2 are indeterminate. Everything below is still pinned to the ORIGINAL commit `dcee60436` and has NOT been edited in place — always cross-reference the triage file's current-location column before citing a line number from this report. Deep reachability/severity re-verification (step 2) and disclosure drafting (step 3) are not yet done — see the follow-up todo task.
