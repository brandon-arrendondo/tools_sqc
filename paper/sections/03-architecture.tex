...
\resizebox{\textwidth}{!}{%
\begin{tikzpicture}[
    stage/.style={rectangle, draw, fill=blue!8, text width=2.9cm,
                  minimum height=2.2cm, align=center, font=\scriptsize,
                  inner sep=3pt},
    fork/.style={rectangle, draw, fill=blue!8, text width=2.9cm,
                 minimum height=1.0cm, align=center, font=\scriptsize,
                 inner sep=3pt},
    absent/.style={rectangle, draw=red!55, dashed, thick, fill=red!3,
                   text width=5.6cm, align=center, font=\scriptsize,
                   inner sep=4pt},
    arrow/.style={->, thick},
]
%% ---- spine (left to right), placed by coordinate for predictability
\node[stage] (src)   at (0,0)    {\textbf{C source tree}\\[2pt]
                                  \texttt{.c}/\texttt{.h} as found\\on disk};
\node[stage] (parse) at (3.6,0)  {\textbf{tree-sitter}\\\textbf{parse}\\[2pt]
                                  concrete syntax\\tree, per file};
%% ---- the fork: file-local facts vs. whole-project facts
\node[fork]  (local) at (7.2,0.68)  {\textbf{local AST facts}\\declarators, lvalues};
\node[fork]  (pre)   at (7.2,-0.68) {\textbf{cross-file prescan}\\summaries, macro\\registry (\texttt{-d})};
\node[stage] (cfg)   at (10.8,0) {\textbf{per-function CFG}\\[2pt]
                                  null-state dataflow\\value-range analysis\\
                                  dead-branch pruning\\init-state tracking};
\node[stage] (rules) at (14.4,0) {\textbf{rule evaluation}\\[2pt]
                                  311 CERT C rules\\provenance gates\\
                                  taint summaries};
\node[stage] (out)   at (18.0,0) {\textbf{diagnostics}\\[2pt]
                                  SARIF 2.1.0, JSON,\\CSV, XLSX\\
                                  CLI, TUI, diff mode};
\draw[arrow] (src)   -- (parse);
\draw[arrow] (parse.east) -- (local.west);
\draw[arrow] (parse.east) -- (pre.west);
\draw[arrow] (local.east) -- (cfg.west);
\draw[arrow] (pre.east)   -- (cfg.west);
\draw[arrow] (cfg)   -- (rules);
\draw[arrow] (rules) -- (out);
%% ---- what the pipeline deliberately does not contain
\node[absent] (na) at (5.4,-2.35)
  {\textbf{No stage here requires:}\\
   a compiler, a build system, a\\
   \texttt{compile\_commands.json}, or a preprocessing pass.\\
   Source that does not compile is still analyzed.};
\draw[->, dashed, red!55, thick] (na.north) -- (parse.south);
\end{tikzpicture}}
\caption{SqC analysis pipeline.  Stage~2 forks: file-local AST facts
are collected alongside a whole-project prescan, and both feed CFG
construction, so cross-translation-unit facts reach a rule without a
linker or a compilation database.  The dashed annotation is the
architectural claim of this paper---no stage consumes build
system output, which is what allows SqC to run on incomplete or
non-compiling source (\cref{sec:corpus-maturity}).}
\label{fig:pipeline}
\end{figure*}
