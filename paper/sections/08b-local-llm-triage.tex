...
The separation is explained by the rules' own definitions, and it is
predictable \emph{a priori}.  MSC17-C asks whether a \texttt{case}
...
The operative criterion for enabling a local-model adjudicator on a
given rule is therefore a static property of the rule---whether its
decision procedure is closed over the finding's syntactic
neighbourhood---rather than something that must be discovered
empirically per rule.

\subsection{Context assembly, not model capability}

The failure of the entire \texttt{PRE*} family (PRE00, PRE01, PRE10,
PRE11, PRE12; 40--57\% balanced accuracy) sharpens this into a design
direction.  These are preprocessor rules, and the macro \emph{definition}
that decides them typically lies outside the 50-line window, frequently
in another file.  Their failure is thus a context-assembly failure
rather than a model-capability failure---and SqC already owns the
machinery to repair it: the registry-based macro-expansion engine of
\cref{sec:limitations}, \texttt{ProjectContext}'s cross-file macro
table, and the inter-procedural function summaries that would supply
the callee facts DCL13-C needs.  Supplying a \emph{relevant-context
bundle} assembled by the analyser, instead of a raw line window, is the
natural next experiment; if it moves the \texttt{PRE*} family, the set
of rules amenable to local adjudication grows well beyond MSC17-C.
