...
TP Rate (precision) & 87.1\% \\
Per-file detection rate & 38.0\% \\
Flaw-hit rate & 12.9\% \\
100\% precision CWEs & 43 \\
\bottomrule
\end{tabular}
\caption{SqC v0.4.321 Juliet benchmark summary (fast mode).  The TP
rate describes output cleanliness; the flaw-hit rate---the share of
the suite's 132,406 planted flaw lines SqC flags---describes detection
breadth, and is the binding constraint (\cref{sec:limitations}).}
...
43 CWEs at 100\% precision and the 22 at high precision together are
82\% of scored CWEs, and no CWE falls below 33\%; the 13 zero-detection
...
\midrule
100\% Precision & 43 & 100\% & 9,817 \\
High ($>$50\%) & 22 & 50--99\% & 12,385 \\
Medium (33--50\%) & 1 & 33--50\% & 8 \\
Low ($<$33\%) & 0 & --- & 0 \\
Zero Detection & 13 & --- & 0 \\
\midrule
\textbf{Total} & \textbf{79}\textsuperscript{$\dagger$} & & \textbf{22,210} \\
\bottomrule
\end{tabular}
\caption{CWE distribution by precision tier (fast mode, v0.4.321).
\textsuperscript{$\dagger$}All 79 scanned CWEs; the 13 with zero
detection contribute 0 TP.}
\label{tab:precision-tiers}
\end{table}

\Cref{tab:perfect-cwes} lists the 43 CWEs with 100\% precision (out of
79 scanned; a further 13 are mapped rules with zero Juliet detections
...
\multicolumn{2}{@{}l}{\emph{+ 29 more CWEs}} & 1,387 \\
\midrule
\textbf{Total} & & \textbf{9,817} \\
\bottomrule
\end{tabular}
\caption{CWEs with 100\% precision (zero false positives), v0.4.321.}
...
libcrc & 364 & 40 & 2 \\
lua & 2,616 & 49 & 107 \\
raylib & 4,353 & 1,060 & 469 \\
mosquitto & 2,588 & 277 & 44 \\
curl & 7,201 & 556 & 116 \\
sqlite & 15,718 & 503 & 137 \\
hostap & 27,255 & 1,761 & 1,710 \\
pure-ftpd & 5,339 & 12 & 109 \\
seL4 & 2,619 & --- & --- \\
\midrule
\textbf{Total} & \textbf{68,053} & \textbf{4,258} & \textbf{2,694} \\
\bottomrule
\end{tabular}
\caption{Violation counts (sqc v0.4.325, cppcheck v2.10, clang-tidy
v21.1.6).  Excluding seL4 (no competitor baseline), SqC's total is
65,434 across the other 8 projects --- an $\sim$15--24$\times$
difference vs.\ cppcheck/clang-tidy, reflecting SqC's 311-rule coverage
vs.\ $\sim$20 checks in competitors.  On the original 7 projects alone
--- the only like-for-like comparison --- SqC's total fell 43\%, from
104,733 at v0.4.120 to 60,095 here, despite covering more rules: the
FP-reduction work in \cref{sec:fp-reduction} landing on real-world
code, not just Juliet.}
...
measured precision is 24.3\% (TP 13,294 of 54,691 labeled findings) with
93.7\% recall (13,294 of 14,184 known true positives flagged), at 88.2\%
label coverage (54,691 of 62,028 in-scope findings labeled;
run \texttt{sqc-0.4.325}, run \#226). Over the preceding seven days, on
the same oracle, precision rose from 22.1\% and total findings fell from
69,532 to 62,028 while recall held (94.2\%$\to$93.7\%): roughly 7,500
false positives removed in a week without a corresponding recall cost.

Two qualifications belong with these numbers rather than in a footnote.
First, the recall figure is measured against \emph{known} true
positives --- defects already present in the oracle. No exhaustive
false-negative hunt exists; past audits deliberately scoped their FN
searches to specific bug categories. True recall is unknown and lower
than 93.7\%.  Second, 11.8\% of findings remain unlabeled, concentrated
in the two projects deliberately sampled at 10\% rather than
exhaustively audited (pure-ftpd, seL4). Both label out well below the
corpus average (seL4 9.5\%, sqlite 11.1\% sample precision against
24.3\% aggregate), so completing them should \emph{lower} measured
precision by an estimated 1--2 points. We state that in advance so the
expected dip is not later misread as a regression.

As with the Juliet 100\%-precision figures in \cref{tab:perfect-cwes},
this aggregate should be read scoped to its oracle rather than as a
universal precision claim, and \cref{sec:corpus-maturity} explains why
its \emph{per-rule} decomposition in particular does not support
rule-quality conclusions.
