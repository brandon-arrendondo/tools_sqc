...
for memory safety.  We ran Infer~v1.2.0 on the same Juliet test
files across 10 overlapping CWEs (17{,}232 files).  Overall, Infer
achieves 43.6\% TP rate (4{,}971~TP / 6{,}428~FP).  Infer is
strongest on null dereference (CWE-476: 66.1\%, CWE-690: 60.0\%)
and double-free (CWE-415: 63.0\%), but weaker on buffer overflow
CWEs (33--39\%) and near-zero on use-after-free (CWE-416: 2.0\%).
SqC covers a broader set of defect classes (118 CWEs vs.\ Infer's
$\sim$10) at a comparable aggregate TP rate.

\textbf{Frama-C}~\cite{frama-c} provides formal verification via
abstract interpretation (EVA plugin) and deductive verification (WP).
We ran Frama-C~32.0 (Germanium) EVA on 6 overlapping CWEs
(5{,}430 files).  Overall, Frama-C achieves 61.0\% TP rate
(8{,}609~TP / 5{,}510~FP)---the highest among tested tools.
CWE-197 (numeric truncation) reaches 100\% precision (912~TP, 0~FP),
and CWE-680 (integer overflow to buffer overflow) reaches 82.6\%.
However, Frama-C requires per-function entry points and
GCC-specific configuration, and takes $\sim$6$\times$ longer per file
than SqC.  It covers 6 CWEs here vs.\ SqC's 118.
