...
\textbf{Parallel mode.}  SqC's prescan cache enables external
parallelism: a single prescan pass builds the cross-file context
(function summaries, call graph, null states) and serializes it to
disk.  Multiple worker processes then analyze file subsets
independently, each loading the shared cache via
\texttt{-{}-load-prescan}.  \Cref{tab:parallel} shows the impact on
M1 (8 workers, 19 subdirectory units).
...
\begin{tabular}{@{}lrrr@{}}
\toprule
\textbf{Mode} & \textbf{Wall (s)} & \textbf{LOC/s} & \textbf{Speedup} \\
\midrule
Single-process     & 3{,}470 & 117 & 1.0$\times$ \\
Parallel (cold)    & 1{,}899 & 213 & 1.8$\times$ \\
Parallel (cached)  & 1{,}056 & 383 & 3.3$\times$ \\
\bottomrule
\end{tabular}
\caption{Parallel scan speedup on M1 for sqlite.  ``Cold'' includes
prescan generation; ``cached'' loads a pre-built 3.1\,MB prescan
cache, eliminating 14 minutes of cross-file analysis setup.}
\label{tab:parallel}
\end{table}

The Juliet benchmark (70 CWEs, fast mode) completes in approximately
30 minutes on M1 using per-CWE parallelism.
