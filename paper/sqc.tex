...
We ran Frama-C~32.0 (Germanium) EVA on 6 overlapping CWEs (integer
overflow, null dereference, divide-by-zero, numeric truncation).
On CWE-476, Frama-C achieves 64.2\% TP rate (373~TP / 208~FP).
Frama-C requires per-function entry points and GCC machdep
configuration; its analysis is more precise but takes $\sim$6$\times$
longer per file than SqC.  Full results are pending.
