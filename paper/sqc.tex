...
examples.  libcrc is small enough that its entire shipped, tooling, and
test tree is in scope.  Across the four audited projects, the evaluated
scope is roughly one third smaller than the corresponding whole-repository
line count; for Mosquitto specifically it is smaller by nearly a factor
of three, so the choice of denominator materially changes any per-KLOC
metric.
...
\cmidrule(l){1-5}
\textbf{Subtotal} &        & \textbf{808} & \textbf{297,382} & \\
\midrule
\multicolumn{5}{@{}l}{\emph{Planned}} \\
hostap    & \texttt{dcee604} & --- & --- & \texttt{src/} + \texttt{wpa\_supplicant/} (audit pending) \\
Lua (C99) & \texttt{40b76de} & --- & --- & C99 idiom corpus (\cref{sec:limitations}) \\
ReactOS   & ---              & --- & --- & Windows module subset (TP/FP scope) \\
\bottomrule
\end{tabular}
\caption{Evaluated source scope per project: SLOC counted over the
audited file set, not the whole repository.  Planned rows are reserved
for benchmarks awaiting adjudication (hostap) or under evaluation as
C99-conformance and Windows targets.}
\label{tab:eval-scope}
\end{table}

The \emph{Planned} rows are placeholders for benchmarks that extend the
suite under the same scoping discipline.  The hostapd audit is deferred
pending adjudication budget; its evaluated scope will be the
\texttt{src/} and \texttt{wpa\_supplicant/} trees that the benchmark
already analyzes.  A C99-idiom corpus (Lua) and a Windows-API corpus
(ReactOS) are under evaluation to exercise, respectively, the
C99-conformance rules---which the current C89/C90-style suite cannot
positively validate---and the \texttt{WIN}-category rules
...
throughput is not directly comparable on analysis depth.}
...
five real-world oracles complement Juliet rather than duplicate it:
