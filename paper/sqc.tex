...
sqc's INT rules are currently opt-out; the residual SQLite false
positives motivate a shift toward opt-in provenance gating.}
\label{tab:optout-optin}
\end{table}

sqc's INT rules are currently opt-out, which puts them in direct
...
variable's bound is silently treated as risk.  The indicated direction
is the hybrid that the precision-oriented literature converges on:
gate reporting on operand provenance, reusing sqc's existing
cross-function taint summaries (\cref{sec:prescan}), and then use the
VRA interval bounds as a prove-safe-to-suppress filter over the
survivors---one reproduced study reports such an SMT-backed filter
removing about 68\% of the remaining false alarms~\cite{kim-filter},
though with high per-program variance.  Unlike the IO2BO checkers
above, a general INT30/31/32-C rule has no single allocation
\emph{sink} to gate on; for sqc the arithmetic operation is itself the
sink, so the gate reduces to ``is any operand tainted or
provably-large.''  The cost is recall: opt-in gating can miss an
overflow whose tainted operand the analysis fails to mark, especially
across function boundaries.  That tradeoff is directly measurable on
Juliet, and we treat a held INT detection rate there as the
precondition for deploying the change---consistent with our practice
of validating every change on both benchmarks.
