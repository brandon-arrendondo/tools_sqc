...
41.1\% to 60.2\%, with 32 of 74 CWEs achieving 100\% precision (zero
...
approaching Frama-C's per-CWE precision.  On real-world code, SqC detects
...
\Cref{tab:juliet-summary} summarizes SqC v0.3.84's performance on
...
Test files & 50,038 \\
True Positives & 25,685 \\
False Positives & 16,965 \\
TP Rate (precision) & 60.2\% \\
Per-file detection rate & 42.7\% \\
100\% precision CWEs & 32 \\
\bottomrule
\end{tabular}
\caption{SqC v0.3.84 Juliet benchmark summary (fast mode).}
...
two-thirds of CWEs achieve either perfect or high precision, while
...
\midrule
100\% Precision & 32 & 100\% & 3,342 \\
High ($>$50\%) & 23 & 50--99\% & 19,686 \\
Medium (33--50\%) & 11 & 33--50\% & 2,650 \\
Low ($<$33\%) & 2 & 3--8\% & 7 \\
Zero Detection & 2 & --- & 0 \\
\midrule
\textbf{Total} & \textbf{70}\textsuperscript{$\dagger$} & & \textbf{25,685} \\
\bottomrule
\end{tabular}
\caption{CWE distribution by precision tier (fast mode, v0.3.84).
...
\multicolumn{2}{@{}l}{\emph{+ 19 more CWEs}} & 227 \\
\midrule
\textbf{Total} & & \textbf{3,342} \\
\bottomrule
\end{tabular}
...
positives.  These rules account for 70\% of all TPs.  The FP rates
vary significantly: EXP34-C achieves 14\% FP rate (CFG-based null-state
...
\midrule
INT32-C & 2,826 & 2,031 & 42 & 190, 191, 680 \\
INT31-C & 2,608 & 2,172 & 45 & 195, 194, 197 \\
ARR38-C & 2,531 & 1,542 & 38 & 121, 126, 124 \\
...
INT30-C & 952  & 1,011 & 52 & 190, 191, 680 \\
EXP34-C & 952  & 157  & 14 & 690, 476 \\
\bottomrule
\end{tabular}
\caption{Top 10 rules by true positive volume (fast mode, v0.3.84).}
\label{tab:top-rules}
\end{table}
...
a new baseline of 45.8\%, which has since improved to 60.2\% through
value-range analysis, CFG constant resolution, and targeted per-CWE
FP reduction.
...
\begin{axis}[
    ...
    ymin=40, ymax=62,
    xtick=data,
    xticklabels={v0.2.1, v0.2.7, v0.2.23, v0.3.20, v0.3.37, v0.3.53, v0.3.75, v0.3.84},
...
\addplot[color=blue, mark=*, thick] coordinates {
    (1, 41.1) (2, 44.5) (3, 44.6) (4, 45.8) (5, 48.4) (6, 52.0) (7, 56.3) (8, 60.2)
};
\end{axis}
\end{tikzpicture}
\caption{TP rate progression from baseline (v0.2.1) through current
(v0.3.84).  The gap between v0.2.23 and v0.3.20 reflects the switch
...
401 & Memory Leak                 & 50.7 & 25.7 & 83.9 & 54.7 & --- \\
415 & Double Free                 & 43.4 & 33.8 & 80.0 & 63.0 & --- \\
416 & Use After Free              & 43.9 & 29.6 & \textbf{60.3} & 2.0  & --- \\
476 & Null Pointer Deref          & 58.3 & 39.7 & \textbf{94.3} & 66.1 & 64.2 \\
...
\midrule
\multicolumn{2}{@{}l}{\textbf{Overall TP rate}} & 57.7 & 36.4 & \textbf{91.6} & 43.6 & 61.0 \\
...
SqC v0.3.84; cppcheck 2.10; clang-tidy (LLVM 21.1); Infer v1.2.0; Frama-C 32.0 EVA.
...
violations than SqC (13{,}952 vs.\ 25{,}685 TP).

Second, \textbf{SqC has closed the gap with Frama-C}: SqC's 57.7\%
on the 15 overlapping CWEs approaches Frama-C's 61.0\%, while covering
74 CWEs vs.\ Frama-C's 6.  SqC now matches or exceeds Frama-C on
CWE-369 (53.9\% vs.\ 43.4\%) and CWE-190 (58.6\% vs.\ 58.1\%).

Third, \textbf{SqC wins on two CWEs outright}: CWE-690 (100\%, where
inter-procedural null propagation from return values eliminates all FPs)
and CWE-761 (100\%, structural pattern matching on free-pointer
arithmetic).
...
41.1\% to 60.2\%, organized by the insight that motivated each change.
...
80.7\%.  Fast-mode continued from 45.8\% to 60.2\% TP rate.}
...
inter-procedural summaries---has reached 60.2\% TP rate.  Further
improvements likely require alias analysis or symbolic execution to
push beyond approximately 65\%.

The 2 remaining CWEs with zero detection (\cref{tab:precision-tiers})
reveal residual coverage gaps: CWE-259 (Hard-Coded Password) is blocked
by tree-sitter's inability to expand preprocessor macros in Juliet's
Windows headers, and CWE-328 (Reversible One-Way Hash) requires a new
cryptographic algorithm detection rule.
...
value-range analysis, and inter-procedural function summaries, SqC
achieves a 60.2\% true positive rate on the Juliet benchmark with 32
CWEs at 100\% precision.  On a five-tool comparison, SqC's precision
approaches Frama-C (57.7\% vs.\ 61.0\% on overlapping CWEs) while
...
Future work includes alias analysis for pointer-related rules, expanding
cross-function taint tracking for injection-related CWEs (CWE-78,
CWE-89), and further closing the precision gap with clang-tidy on
buffer overflow CWEs through improved buffer-size inference.
