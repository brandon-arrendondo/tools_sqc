...
improving the true positive rate from 41.1\% to 53.6\%.  Of 74 CWEs
tested, 31 achieve 100\% precision (zero false positives).  On real-world
...
Test files & 50,038 \\
True Positives & 24,408 \\
False Positives & 21,117 \\
TP Rate (precision) & 53.6\% \\
100\% precision CWEs & 31 \\
\bottomrule
\end{tabular}
\caption{SqC v0.3.60 Juliet benchmark summary (fast mode).}
...
\midrule
100\% Precision & 31 & 100\% & 2,539 \\
High ($>$50\%) & 16 & 50--99\% & 7,519 \\
Medium (33--50\%) & 19 & 33--50\% & 14,343 \\
Low ($<$33\%) & 2 & 3--8\% & 7 \\
Zero Detection & 2 & --- & 0 \\
\midrule
\textbf{Total} & \textbf{70}\textsuperscript{$\dagger$} & & \textbf{24,408} \\
\bottomrule
\end{tabular}
\caption{CWE distribution by precision tier (fast mode, v0.3.60).
\textsuperscript{$\dagger$}4 CWEs had no test files.}
\label{tab:precision-tiers}
\end{table}

\Cref{tab:perfect-cwes} lists the 31 CWEs with 100\% precision.  These
...
\multicolumn{2}{@{}l}{\emph{+ 18 more CWEs}} & 187 \\
\midrule
\textbf{Total} & & \textbf{2,539} \\
\bottomrule
\end{tabular}
...
a new baseline of 45.8\%, which has since improved to 53.6\%.
...
\begin{axis}[
    ...
    ymin=40, ymax=56,
    xtick=data,
    xticklabels={v0.2.1, v0.2.7, v0.2.23, v0.3.20, v0.3.37, v0.3.53, v0.3.60},
...
\addplot[color=blue, mark=*, thick] coordinates {
    (1, 41.1) (2, 44.5) (3, 44.6) (4, 45.8) (5, 48.4) (6, 52.0) (7, 53.6)
};
\end{axis}
\end{tikzpicture}
\caption{TP rate progression from baseline (v0.2.1) through current
(v0.3.60).  The gap between v0.2.23 and v0.3.20 reflects the switch
...
\Cref{tab:competitor} compares SqC against Facebook Infer~v1.2.0 and
Frama-C~32.0 (Germanium EVA) on overlapping Juliet CWEs.  Each tool
was run on the same test files with TP/FP classified using identical
Juliet ground truth (procedure names and \texttt{OMITBAD}/\texttt{OMITGOOD}
guards).
...
\begin{tabular}{@{}llrrr@{}}
\toprule
\textbf{CWE} & \textbf{Description} & \textbf{SqC} & \textbf{Infer} & \textbf{Frama-C} \\
\midrule
\multicolumn{5}{@{}l}{\emph{Memory Safety (Infer overlap)}} \\
476 & Null Pointer Deref    & 50.8 & \textbf{66.1} & 64.2 \\
690 & Null Deref from Return & \textbf{94.6} & 60.0 & --- \\
415 & Double Free           & 43.4 & \textbf{63.0} & --- \\
401 & Memory Leak           & 50.7 & \textbf{54.7} & --- \\
416 & Use After Free        & \textbf{43.9} & 2.0  & --- \\
761 & Free Not at Start     & \textbf{100.0} & 58.1 & --- \\
121 & Stack Buffer Overflow & \textbf{48.8} & 38.8 & --- \\
122 & Heap Buffer Overflow  & \textbf{58.5} & 37.8 & --- \\
124 & Buffer Underwrite     & \textbf{52.1} & 33.9 & --- \\
127 & Buffer Underread      & \textbf{55.6} & 33.9 & --- \\
\midrule
\multicolumn{5}{@{}l}{\emph{Value Analysis (Frama-C overlap)}} \\
190 & Integer Overflow      & 47.4 & --- & \textbf{58.1} \\
191 & Integer Underflow     & 45.5 & --- & \textbf{63.1} \\
197 & Numeric Truncation    & 70.2 & --- & \textbf{100.0} \\
369 & Divide by Zero        & 36.9 & --- & \textbf{43.4} \\
680 & Int Overflow $\to$ BOF & 53.9 & --- & \textbf{82.6} \\
\midrule
\multicolumn{2}{@{}l}{\textbf{CWEs tested}} & \textbf{118} & 10 & 6 \\
\bottomrule
\end{tabular}
\caption{TP rate (\%) comparison on overlapping Juliet CWEs.
\textbf{Bold} = highest per row.  ``---'' = CWE not tested.
SqC v0.3.74; Infer v1.2.0 (17{,}232 files); Frama-C 32.0 EVA (5{,}430 files).}
\label{tab:competitor}
\end{table}

Three patterns emerge.  First, Infer and Frama-C achieve higher precision
on their core competencies: Infer on null dereference (66.1\% vs.\
50.8\%) and double-free (63.0\% vs.\ 43.4\%); Frama-C on numeric
truncation (100\% vs.\ 70.2\%) and integer overflow (58--63\% vs.\
45--47\%).  This reflects their use of separation logic and abstract
interpretation, respectively.

Second, SqC outperforms Infer on CWEs where Juliet's cross-function
patterns challenge Infer's procedure-level analysis: CWE-690 (94.6\%
vs.\ 60.0\%), CWE-761 (100\% vs.\ 58.1\%), and all four buffer
overflow CWEs (49--59\% vs.\ 34--39\%).

Third, the coverage gap is stark: SqC tests 118 CWEs spanning 17
CERT~C categories, while Infer and Frama-C cover 10 and 6 CWEs
respectively on this benchmark.  A hypothetical ``best of all three''
ensemble would cover more CWEs with higher per-CWE precision, suggesting
that these tools are complementary rather than competitive.
...
FP reduction (full-suite measurement), organized by the insight that motivated each change.
...
Future work includes expanding value-range analysis and improving
cross-function taint tracking for injection-related CWEs (CWE-78,
CWE-89).  Inter-procedural taint propagation would also improve
coverage for CWE-789 and CWE-114, where only intra-procedural
variants are currently detected.
