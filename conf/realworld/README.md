...
A rule with **no entry at all** in a manifest never runs (the scan iterates
`RuleManifest::enabled_rules()`), and nothing reports the gap — libcrc's
manifest was missing `MSC04-C` and `MSC07-C`, so the suite's exhaustively
adjudicated reference oracle had been scanning with two rules silently off. All
nine now carry an entry for every rule in `rules_templates/rules-all.toml`;
tools_sqc task 997 tracks a check that keeps it that way.
...
|-----------|---------------------------|--------------|
| libcrc    | `libcrc-rules.toml`       | Was **full** — every enabled-rule finding labelled (422; 13 TP / 409 FP). `PRE00-C` and `DCL08-C` newly enabled here. See `data/precision_audit/libcrc/`. |
