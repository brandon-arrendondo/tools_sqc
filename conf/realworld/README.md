...
Every codebase must have one. There is no shared fallback base: a `CODEBASES`
entry with no `manifest` raises rather than scanning under a policy nobody
wrote for it.
...
suite-wide decision has to be written into all nine files.

**There is no curated shared base to write it into as well.** There used to be
one — `rules_templates/rules-benchmark.toml`, generated by `build.rs` with a
hardcoded 13-rule noise block — and it was retired because it served two
masters: it was also the full-mode Juliet manifest, so a real-world noise
judgement silently changed what a `--full` Juliet run measured, which is how
that block went 13 rules deep without anyone noticing. Start a new manifest
from `rules_templates/rules-all.toml` (generated by `build.rs`, the same
manifest aurora-lint embeds as its own default) and make every deviation from
it an explicit `enabled = false` here.

**Enforced by `scripts/check_realworld_manifests.py`** (pre-commit hook
`check-realworld-manifests`, and runnable by hand). It fails, naming the file
and the ids, when a manifest here is missing a block for a rule in
`rules_templates/rules-all.toml`, when it carries a block for a rule the base
no longer has, or when an `enabled = false` has no comment. So adding a rule
under `src/rules/cert_c/` fails until all nine manifests carry a decision for
it — which is also how a new rule stops being silently dark on the real-world
suite.
...
2. Decide categorical disables → write `conf/realworld/<cb>-rules.toml` (start
   from `rules_templates/rules-all.toml`; every disable needs a comment
