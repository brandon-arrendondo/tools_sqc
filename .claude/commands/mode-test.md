...
- **UNLOCKED**: C test files (`*/tests/*.c`)
- **LOCKED**: Rule implementations (`*_c.rs`), utility files (`src/utility/cert_c/*.rs`), mod.rs files
...
├── fail/
│   ├── wiki_*.c         # From CERT C wiki (READ-ONLY - regenerated by scraper)
│   └── testcases_*.c    # Manual test cases (EDIT THESE)
└── pass/
    ├── wiki_*.c         # From CERT C wiki (READ-ONLY - regenerated by scraper)
    └── testcases_*.c    # Manual test cases (EDIT THESE)
```
