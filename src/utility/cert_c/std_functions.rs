...
/// pointer/offset arithmetic. Distinct from [`is_full_range_return_function`]
/// (generic libc parsers/RNG): this is the deserialization category the SQLite
/// real-world audit flagged as a dominant true-positive source (untrusted
/// varint drives a read/offset without a bound check).
///
/// Scope is deliberately limited to **varint decoders**. Fixed-width byte
/// readers (`get2byte`/`get4byte`) and stored-value/size accessors
/// (`sqlite3_column_bytes`, `sqlite3_value_int`, ...) were measured to add many
/// false positives — they taint sizes/lengths that callers subsequently bound-
/// check, and we do not yet model that sanitizer — while recovering *no*
/// additional true positives in the SQLite corpus. Varint returns, by contrast,
/// are consumed raw as offsets. The names cover SQLite/FTS/session varint
/// helpers; the pattern generalizes to protobuf-style codecs and on-disk/wire
/// formats.
