...
mod tests {
    ...
    fn test_unjoined_thread() {
        ...
        assert!(
            !violations.is_empty(),
            "Should detect unjoined/undetached thread"
        );
    }

    #[test]
    fn test_joined_thread() {
        let code = r#"
            #include <threads.h>
            int main(void) {
                thrd_t thr;
        ...
        assert!(
            violations.is_empty(),
            "Should not flag joined thread: {:?}",
            violations
        );
    }

    #[test]
    fn test_self_detaching_thread() {
        let code = r#"
            #include <threads.h>
            int message_print(void *ptr) {
            ...
            int main(void) {
                thrd_t thr;
                thrd_create(&thr, message_print, NULL);
                return 0;
            }
        "#;
        ...
        assert!(
            violations.is_empty(),
            "Should not flag self-detaching thread: {:?}",
            violations
        );
    }
}
