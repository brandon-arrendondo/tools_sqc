...
mod tests {
    ...
    #[test]
    fn test_recv_without_conversion() {
        let code = r#"
            void func(int sock) {
                uint32_t num;
                recv(sock, (void *)&num, sizeof(uint32_t), 0);
                printf("%u\n", num);
            }
        "#;
        ...
        assert!(
            !violations.is_empty(),
            "Should detect recv without byte order conversion"
        );
    }

    #[test]
    fn test_recv_with_ntohl() {
        let code = r#"
            void func(int sock) {
                uint32_t num;
                recv(sock, (void *)&num, sizeof(uint32_t), 0);
        ...
        assert!(
            violations.is_empty(),
            "Should not flag recv with ntohl conversion: {:?}",
            violations
        );
    }

    #[test]
    fn test_recv_uint16_without_conversion() {
        let code = r#"
            void func(int sock) {
                uint16_t port;
                recv(sock, &port, sizeof(uint16_t), 0);
                printf("%u\n", port);
            }
        "#;
        ...
        assert!(
            !violations.is_empty(),
            "Should detect recv on uint16_t without ntohs"
        );
    }
}
