...
mod tests {
    ...
    fn test_sizeof_on_char_buffer() {
        ...
        assert!(
            !violations.is_empty(),
            "Should detect sizeof() on char buffer in fwrite"
        );
    }
    ...
    fn test_strlen_plus_one() {
        ...
        assert!(
            violations.is_empty(),
            "Should not flag proper strlen+1 usage: {:?}",
            violations
        );
    }
    ...
    fn test_binary_data() {
        ...
        // Should not flag binary data (non-char buffers)
        assert!(
            violations.is_empty(),
            "Should not flag binary data fwrite: {:?}",
            violations
        );
    }
}
