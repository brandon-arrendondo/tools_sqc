...
mod tests {
    ...
    fn test_equals_one_comparison() {
        ...
        assert!(
            !violations.is_empty(),
            "Should detect == 1 comparison with function call"
        );
    }

    #[test]
    fn test_not_validate_function() {
        let code = r#"
            void processRequest(User usr) {
                if (!validateUser(usr)) {
                    return;
        ...
        assert!(
            !violations.is_empty(),
            "Should detect implicit boolean test on validate function"
        );
    }

    #[test]
    fn test_explicit_not_equals_zero() {
        let code = r#"
            void processRequest(User usr) {
                if (is_banned(usr) != 0) {
                    return;
        ...
        assert!(
            violations.is_empty(),
            "Should not flag explicit != 0 test: {:?}",
            violations
        );
    }

    #[test]
    fn test_explicit_equals_zero() {
        let code = r#"
            void processRequest(User usr) {
