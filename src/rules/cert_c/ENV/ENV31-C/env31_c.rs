...
mod tests {
    ...
    fn test_envp_after_setenv() {
        ...
        assert!(
            !violations.is_empty(),
            "Should detect envp usage after setenv"
        );
    }

    #[test]
    fn test_envp_after_putenv_s() {
        let code = r#"
            int main(int argc, const char *argv[], const char *envp[]) {
        ...
        assert!(
            !violations.is_empty(),
            "Should detect envp usage after _putenv_s"
        );
    }

    #[test]
    fn test_environ_after_setenv() {
        let code = r#"
            extern char **environ;
            int main(void) {
                if (setenv("MY_NEW_VAR", "new_value", 1) != 0) {
        ...
        assert!(
            violations.is_empty(),
            "Should not flag environ usage: {:?}",
            violations
        );
    }

    #[test]
    fn test_envp_no_modification() {
        let code = r#"
            int main(int argc, char *argv[], char *envp[]) {
                puts(envp[0]);
                return 0;
        ...
        assert!(
            violations.is_empty(),
            "Should not flag envp when no environment modification: {:?}",
            violations
        );
    }
}
