...
mod tests {
    ...
    fn test_exit_in_handler() {
        ...
        assert!(
            !violations.is_empty(),
            "Should detect exit() call in exit handler"
        );
    }

    #[test]
    fn test_longjmp_in_handler() {
        let code = r#"
            jmp_buf env;
            void exit_handler(void) {
                longjmp(env, 1);
        ...
        assert!(
            !violations.is_empty(),
            "Should detect longjmp() call in exit handler"
        );
    }

    #[test]
    fn test_normal_return_handler() {
        let code = r#"
            void exit_handler(void) {
                // cleanup
                return;
        ...
        assert!(
            violations.is_empty(),
            "Should not flag normal return: {:?}",
            violations
        );
    }

    #[test]
    fn test_non_handler_exit() {
        let code = r#"
        ...
        assert!(
            violations.is_empty(),
            "Should not flag exit() in non-handler functions: {:?}",
            violations
        );
    }
}
