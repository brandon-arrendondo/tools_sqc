...
mod tests {
    ...
    fn test_struct_without_memset() {
        ...
        assert!(
            !violations.is_empty(),
            "Should detect struct passed without zeroing"
        );
    }

    #[test]
    fn test_struct_with_memset() {
        let code = r#"
            struct test { int a; char b; int c; };
            void do_stuff(void *usr_buf) {
                struct test arg;
        ...
        assert!(
            violations.is_empty(),
            "Should not flag struct zeroed with memset: {:?}",
            violations
        );
    }

    #[test]
    fn test_no_trust_boundary() {
        let code = r#"
            struct test { int a; char b; int c; };
            void do_stuff(void) {
                struct test arg = {.a = 1, .b = 2, .c = 3};
        ...
        assert!(
            violations.is_empty(),
            "Should not flag non-trust-boundary functions"
        );
    }
}
