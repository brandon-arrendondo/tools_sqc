...
mod tests {
    ...
    fn test_repeated_magic_number() {
        ...
        assert!(
            !violations.is_empty(),
            "Should detect repeated magic number 256"
        );
    }

    #[test]
    fn test_array_magic_number() {
        let code = r#"
            void func(void) {
                char buffer[1024];
            }
        ...
        assert!(
            !violations.is_empty(),
            "Should detect magic number in array size"
        );
    }
    ...
    fn test_acceptable_values() {
        ...
        // Should not flag 0 and 1 as they are acceptable
        assert!(
            violations.is_empty(),
            "Should not flag 0 and 1 as magic numbers: {:?}",
            violations
        );
    }

    #[test]
    fn test_symbolic_constant_compliant() {
        let code = r#"
            enum { BUFFER_SIZE = 256 };
            void func(void) {
                char buffer[BUFFER_SIZE];
        ...
        assert!(
            violations.is_empty(),
            "Should not flag symbolic constants: {:?}",
            violations
        );
    }

    #[test]
    fn test_single_magic_number() {
        let code = r#"
            void func(void) {
                if (age >= 18) { }
            }
        ...
        // Single occurrence should be flagged as per CERT C wiki
        assert!(
            !violations.is_empty(),
            "Single magic number occurrence should be flagged"
        );
    }
}
