...
impl CertRule for Dcl38C {
    fn rule_id(&self) -> &'static str { "DCL38-C" }
    ...
    fn cert_id(&self) -> &'static str { "DCL38-C" }
            ...
            if text.contains("[1]") {
                if let Some(parent) = node.parent() {
                    if parent.kind() == "field_declaration_list" {
                        ...
                        let fields: Vec<_> = parent.children(&mut cursor)
                        ...
                        if let Some(last) = fields.last() {
                            if last.id() == node.id() {
                                violations.push(RuleViolation {
                                    ...
                                    message: "Flexible array member should use [] not [1]".to_string(),
