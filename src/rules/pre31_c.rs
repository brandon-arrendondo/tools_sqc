...
"#;
    ...
    #[test]
    fn test_pre31c_detects_multiple_side_effects() {
        let rule = Pre31C;
        let mut parser = CParser::new().unwrap();

        // Test case: Multiple arguments with side effects
        let source = r#"
#define SWAP(a, b) do { typeof(a) temp = a; a = b; b = temp; } while(0)

void func(void) {
    int x = 1, y = 2;
    SWAP(++x, --y);  // Should trigger violations for both arguments
}
"#;
        ...
        // Should detect side effects in both arguments
        assert!(!violations.is_empty(), "Should detect side effects in SWAP arguments");

        // Look for violations mentioning side effects
        let side_effect_count = violations.iter()
            .filter(|v| v.message.contains("side effect"))
            .count();
        assert!(side_effect_count > 0, "Should detect at least one side effect");
    }

    #[test]
    fn test_pre31c_accepts_safe_function_calls() {
        let rule = Pre31C;
        let mut parser = CParser::new().unwrap();

        // Test case: Regular function calls (not macros)
        let source = r#"
int regular_function(int x);

void func(int n) {
    int result = regular_function(++n);  // Should not trigger violation - not a macro
}
"#;
        ...
        // Should not flag regular function calls, only unsafe macros
        let macro_violations: Vec<_> = violations.iter()
            .filter(|v| v.message.contains("Unsafe macro"))
            .collect();
        assert!(macro_violations.is_empty(), "Should not flag regular function calls");
    }
}